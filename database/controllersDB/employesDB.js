...
exports.createtable_publication = (req, res) => {
    let sql = 'CREATE TABLE publication(id int AUTO_INCREMENT, titre VARCHAR(100) NOT NULL, texte VARCHAR(500) NOT NULL, employeID int NOT NULL,date DATETIME NOT NULL, CONSTRAINT fk_publication_emplyeID FOREIGN (employeID) REFERENCES employes(id), PRIMARY KEY(id));';
    db.query(sql, err => {
        if (err) {
...
exports.createtable_commentaire = (req, res) => {
    let sql = 'CREATE TABLE commentaire(id int AUTO_INCREMENT, texte VARCHAR(500) NOT NULL, employeID int NOT NULL, publicationID int NOT NULL, date DATETIME NOT NULL, CONSTRAINT fk_commentaire_emplyeID FOREIGN (employeID) REFERENCES employes(id),CONSTRAINT fk_commentaire_publicationID FOREIGN (publicationID) REFERENCES publication(id), PRIMARY KEY(id));';
    db.query(sql, err => {
        if (err) {
